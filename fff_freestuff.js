/* fff_freestuff.js — "Gary's Free Parts Bin".
   Float something free into the community bin, first tap grabs it, karma rule keeps it honest.
   Featured drop: Gary's Garage. House rules: honest-state (demo bin clearly labeled),
   localStorage Step-1, bouncing FAB + overlay. */
(function(){
  'use strict';
  ...
  function openOv(){
    ...
    var feat='<div class="ffree-feat"><div class="ffree-feat-h">🧰 Featured drop</div>'+
      ...
      '<button class="ffree-btn" id="ffreeGary">Ask Steve about Gary’s drop →</button></div>';
    var rows=BIN.map(function(b){ var got=!!GRABBED[b.id];
    ...
    ov.innerHTML='<div class="ffree-sheet">'+
      '<div class="ffree-top"><div><div class="ffree-title">Gary’s Free Parts Bin</div><div class="ffree-sub">Free stuff, floated by the community</div></div>'+
        '<button class="fp-x" id="ffreeX">✕</button></div>'+
      feat+
      '<div class="ffree-sect">In the bin</div>'+rows+
      '<button class="ffree-btn ghost" id="ffreeGive">＋ Float something free</button>'+
      '<div class="ffree-karma">Karma rule: grab it and actually use it — or pass it back so someone else can. Grab &amp; ghost and you sit out the bin until you make it right.</div>'+
      '<div class="fp-note">Demo bin — real free listings appear here as members float them. No payments run through the Parts Bin.</div>'+
    '</div>';
    ov.classList.add('on');
    ...
    ov.querySelectorAll('.ffree-grab').forEach(function(b){ b.onclick=function(){ GRABBED[b.dataset.id]=!GRABBED[b.dataset.id]; save('fff.free.grabbed',GRABBED); openOv();
      if(window.toast) toast(GRABBED[b.dataset.id]?'🧰 Grabbed — arrange pickup in the thread.':'Released back to the bin.'); }; });
  ...
  function mountFab(){
    ...
    var b=document.createElement('button'); b.id='ffreeFab'; b.className='ffree-fab'; b.title='Gary’s Free Parts Bin';
